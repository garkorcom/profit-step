...
export interface GTDTask {
    ...
    /** ID задач-блокеров, от которых зависит выполнение текущей */
    dependsOn?: string[];

    /** Является ли задача вехой (событием без длительности) */
    isMilestone?: boolean;

    /** Кастомный цвет карточки на диаграмме Ганта */
    ganttColor?: string;

    /** Первоначально запланированная дата начала (для план-факт анализа) */
    ...
    /**
     * ID родительской задачи (для связи подзадач с глобальной задачей/проектом)
     * Если указан — задача является подзадачей и НЕ отображается на главной Kanban-доске.
     */
    parentTaskId?: string;

    /**
     * Денормализованный флаг для быстрой фильтрации в Firestore-запросах.
     * true = задача является подзадачей (parentTaskId существует).
     * Позволяет использовать where('isSubtask', '!=', true) без Composite Index.
     */
    isSubtask?: boolean;

    /**
     * Бюджетная сумма (из сметы) для этой задачи/работы.
     * Используется для расчёта: budgetAmount * (progressPercentage / 100) = выполненная сумма.
     */
    budgetAmount?: number;

    /**
     * Процент выполнения работы (0-100).
