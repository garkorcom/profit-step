...
2. Прочитай `PROJECT_MAP.md` (карта проекта) и `README.md`.
3. Запусти `git status` в `/Users/denysharbuzov/Projects/profit-step` — если там 100+ uncommitted файлов, значит кто-то (другой агент или Денис) работает параллельно. **Не делай `git add -A`** пока не разберёшься.
4. Проверь `git log --oneline -10` — понять в каком состоянии ветка.
5. Посмотри `src/pages/dashbord-for-client/SPEC.md` если задача связана с client dashboard / portal.
