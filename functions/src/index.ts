...
export const testEmail = functions.https.onCall(async (data, context) => {
  ...
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Требуется аутентификация для отправки тестового email'
    );
  }
  ...
  try {
    ...
    const emailHost = config.email?.host || 'smtp-relay.brevo.com';
    const emailPort = parseInt(config.email?.port || '587', 10);
    const emailFrom = config.email?.from || emailUser;
...
Powered by Firebase Functions + Brevo SMTP
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Ошибка отправки тестового email: ${error.message}`,
      {
        code: error.code,
