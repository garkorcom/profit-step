...
- **Эндпоинты:**
  ...
  - [Системные (Health, Dashboard)](#18-системные)
  - [Inventory — Склады и Запасы](#19-inventory--склады-и-запасы)
...
- `status` — фильтр по статусу (new, contacted, qualified, customer, active)

**Response `200`:**
```json
{
  "clients": [
    { "clientId": "abc123", "name": "Steve Jobs LLC", "address": "1 Infinite Loop", "phone": "+1-555-000", "email": "tim@apple.com", "status": "customer", "type": "commercial" }
  ],
  "count": 1,
  "total": 15
}
...
### `GET /api/clients/:id` — Полный профиль клиента

Возвращает клиента с 6 параллельными агрегациями: проекты, задачи, расходы, трекинг, сметы, объекты.

**Response `200`:**
```json
{
  "client": { "id": "abc123", "name": "Steve Jobs LLC", "..." : "..." },
  "projects": [{ "id": "p1", "name": "Ремонт офиса", "status": "active" }],
  "tasks": {
    "total": 12,
    "byStatus": { "inbox": 3, "next_action": 5, "completed": 4 },
    "items": [{ "id": "t1", "title": "Закупка", "status": "next_action", "priority": "high" }]
  },
  "costs": { "total": 5400.50, "count": 8, "byCategory": { "materials": 3200, "labor": 2200.50 } },
  "timeTracking": { "totalMinutes": 1200, "totalHours": 20.0, "totalEarnings": 800.00, "sessionCount": 15 },
  "estimates": [{ "id": "e1", "status": "approved", "total": 15000 }],
  "sites": [{ "id": "s1", "address": "123 Main St", "status": "active" }]
}
```
...
### `POST /api/gtd-tasks/batch-update` — Массовое обновление задач
...
```
- `taskIds` — до 50 ID задач
- `update` — поля для обновления: `status`, `priority`, `assigneeId`, `assigneeName`

**Response `200`:**
```json
{ "updated": 3, "notFound": ["fake-id"] }
```
`notFound` присутствует только если есть несуществующие ID.
...
**Не требует авторизации.** Публичный endpoint для мониторинга.
...
Возвращает агрегированную информацию за один запрос: активные сессии, задачи на сегодня, расходы, открытые сметы.

**Response `200`:**
```json
{
  "activeSessions": [
    { "id": "s1", "employeeName": "Иван", "task": "Штукатурка", "startTime": "2026-04-02T09:00:00Z", "status": "active", "clientName": "Steve LLC" }
  ],
  "activeSessionCount": 1,
  "tasksDueToday": [
    { "id": "t1", "title": "Закупить цемент", "status": "next_action", "priority": "high", "clientName": "Steve LLC", "assigneeName": "Иван" }
  ],
  "tasksDueTodayCount": 1,
  "recentCosts": [
    { "id": "c1", "amount": 350, "category": "materials", "description": "Цемент", "clientName": "Steve LLC", "createdAt": "2026-04-01T15:00:00Z" }
  ],
  "openEstimates": { "count": 3, "totalValue": 45000 },
  "totalClients": 28,
...
## 19. Inventory — Склады и Запасы

> **Важно:** все warehouse endpoints живут под префиксом `/api/inventory/warehouses`,
> **не** `/api/warehouses`. Запросы к `/api/warehouses` вернут `404`.

Модуль поддерживает два типа складов:
- `physical` — стационарный склад (location, address, клиент/проект)
- `vehicle` — машина бригады (требует `licensePlate`)

Обе концепции хранятся в одной Firestore коллекции `warehouses`. Existing документы без `type` читаются как `physical` (on-read default). Архивация soft-delete через поле `archived`.

### `POST /api/inventory/warehouses` — Создать склад
...
```json
{
  ...
  "description": "Tools and materials storage",
...
- `type` — optional, `'physical'` (default) | `'vehicle'`
- `licensePlate` — **required if** `type === 'vehicle'` (non-empty)
...
**Ошибки:**
- `400` — Zod validation failed (например, vehicle без licensePlate)
- `200 + deduplicated: true` — если передан `idempotencyKey` совпадающий с недавним запросом (24h TTL)
...
- `includeArchived` — `true` чтобы включить archived
...
### `GET /api/inventory/warehouses/:id` — Детали склада + items

Возвращает warehouse + список `inventory_items` (до 200) с данным `warehouseId`.

**Query params:**
- `includeArchived` — `true` чтобы получить archived warehouse (иначе `404`)
...
```json
{
  ...
  "items": [
    { "id": "item_1", "name": "Drywall screws", "quantity": 500, "unit": "pcs", "category": "hardware", "minStock": 100, "barcode": null, "photoUrl": null, "createdAt": "2026-04-02T10:15:00.000Z" }
  ],
  "itemCount": 1
}
```
...
**Ошибки:**
- `404` — не найден или архивирован (без `includeArchived=true`)

### `PATCH /api/inventory/warehouses/:id` — Обновить склад

Partial update. Должен содержать минимум одно поле.

**Body (пример):**
```json
{
  "name": "New Name",
  "licensePlate": "NY-XYZ-9999"
}
```

**Разрешённые поля:** `name`, `clientId`, `projectId`, `address`, `description`, `type`, `location`, `licensePlate`

**Server-side инвариант:** после merge с existing state, `type === 'vehicle'` требует non-empty `licensePlate`. Это значит:
- Можно поменять `type` на `vehicle` отдельным запросом, если у документа уже есть `licensePlate`
- Нельзя убрать `licensePlate` у vehicle документа
- Нельзя сделать `vehicle` без licensePlate в одном запросе

**Response `200`:**
```json
{ "warehouseId": "wh_abc123", "updated": true }
```

**Ошибки:**
- `400` — vehicle invariant violated
- `404` — warehouse не найден
- `409` — warehouse архивирован (нельзя редактировать)

### `DELETE /api/inventory/warehouses/:id` — Архивировать склад

Soft-delete — устанавливает `archived: true`, сохраняет документ и все references из `inventory_items.warehouseId`.

**Query params:**
- `force=true` — обойти проверку стока (см. ниже)

**Safety check:** endpoint **отказывается** архивировать warehouse если в нём есть `inventory_items` с `quantity > 0`, если только не передан `?force=true`. Это защищает от случайного архивирования склада с остатками.

**Response `200`:**
```json
{ "warehouseId": "wh_abc123", "archived": true }
```

**Ошибки:**
- `404` — warehouse не найден
- `409` — warehouse уже архивирован
- `409` — есть stocked inventory_items (передай `?force=true` чтобы обойти)

### Inventory Items (краткий справочник)

Под тем же префиксом есть полный CRUD для позиций склада. Каждый `inventory_item` привязан к warehouse через `warehouseId`:
- `POST /api/inventory/items` — добавить позицию
- `GET /api/inventory/items?warehouseId=...` — список
- `PATCH /api/inventory/items/:id` — обновить
- `DELETE /api/inventory/items/:id` — удалить
- `POST /api/inventory/transactions` — списания/приходы/переводы
- `POST /api/inventory/transactions/task` — bulk материалы для задачи
- `GET /api/inventory/transactions` — история движений
- `POST /api/inventory/norms` — создать норму списания
- `GET /api/inventory/norms` — список норм
- `POST /api/inventory/write-off-by-norm` — списание по норме

См. исходник в `functions/src/agent/routes/inventory.ts` для полных схем.
